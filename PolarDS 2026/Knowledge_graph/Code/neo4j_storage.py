...
class Neo4jConnector:
    ...
    def store_keywords_and_relations(self, keywords, relationships, datasets=None):
        ...
        with self.driver.session() as session:
            ...
            # Create all keyword nodes FIRST. Dataset-variable linking below
            # MATCHes on Keyword nodes -- if that ran first (the previous
            # order), the MATCH always found nothing since no Keyword node
            # existed yet, so _mark_as_variable/_link_dataset_variable
            # silently no-opped even for variables that exactly matched an
            # extracted keyword (confirmed empirically, a pre-existing bug
            # independent of the match condition below).
            for key in keywords:
            ...
            # Link each originally-extracted keyword to all datasets
            for key in keywords:
                cleaned_key = clean_text(key)
