...
# This module lives in Causal_graph/Code/, but keywords_extraction.py (and
# its own variable_filter.py dependency) live in the sibling top-level
# folder Knowledge_graph/Code/, so it must be added to sys.path first.
...
# Longest a single keyword candidate is allowed to be, in words, before
# it's dropped as malformed -- see _drop_long_candidates() below. Guards
# against the Keywords-section parser's line-join-then-comma-split logic
# occasionally producing a single ~30-word run-on "keyword" when a paper's
# declared list is formatted unusually (no delimiter the parser expects).
MAX_KEYWORD_WORDS = 8

# How much wider the candidate pool is requested when re-fetching after a
# declared Keywords: section is found (see enhanced_extract_keywords()
# below) -- 3x was chosen empirically as enough slack to surface genuine
# supplementary body-text variables without pulling in excessive noise.
WIDE_K_MULTIPLIER = 3
...
def _dedupe_preserve_order(keywords):
    """Remove case/whitespace-insensitive duplicates while keeping the
    first occurrence's original spelling and position. Order matters here:
    callers merge the declared-keyword list with supplementary body-text
    terms and rely on the declared list's original ranking staying intact
    at the front."""
    seen = set()
    result = []
    for kw in keywords:
...
def _drop_long_candidates(keywords, max_words=MAX_KEYWORD_WORDS):
    """Drop any candidate longer than max_words -- filters out the
    malformed run-on "keywords" MAX_KEYWORD_WORDS guards against above."""
    return [kw for kw in keywords if len(kw.split()) <= max_words]


def _declared_vocab(declared_keywords):
    """Builds a flat set of individual words (lowercased) appearing
    anywhere across the declared Keywords: list, e.g. {"sea", "ice",
    "extent", ...} from ["sea ice extent"]. Used by
    _drop_declared_fragments() below to catch supplementary candidates
    that are just a subset of words already declared (e.g. "ice"/"sea"
    out of "sea ice")."""
    vocab = set()
...
def enhanced_extract_keywords(input_text, k):
    ...
    section) — TF-IDF (term-frequency scoring), YAKE (statistical
    unsupervised extraction), and KeyBERT (embedding-based ranking), all
    three implemented and combined in keywords_extraction.py, not here —
    extract_keywords() scores candidates via keywords_extraction.scale(), a
    softmax over however many candidates it is given — asking for 3x more
    candidates therefore reranks the same top terms rather than just
    enlarging the pool, which can surface low-value single-word terms
    (e.g. "causal", "model", "pcmci") above genuine variables. The
    algorithmic path is therefore called with the plain k and its result
    returned untouched, matching plain keywords_extraction.process()'s
    output exactly.
    """
