...
"""
    ...
    try:
        ...
        # min_confidence=0.4 is this code's own post-filter cutoff, independent
        # of the LLM's self-reported 1.0/0.6/0.3 CONFIDENCE bands explained in
        # the prompt above -- it exists because self-reported confidence
        # clusters near 1.0 regardless of soundness (see the embedding_confidence
        # comment below), so a fixed floor here discards only the pairs the LLM
        # itself flagged as indirect/uncertain (0.3), not a tuned statistical
        # threshold.
        ...
        if not causal_rels and content.strip():
            ...
            print(content)
            # Same fixed 0.4 floor as the first pass above -- see that call's comment.
            causal_rels = _parse_causal_output(content, valid_nodes_lower, min_confidence=0.4)
...
def _confidence_to_color(confidence: float) -> str:
    """
    Maps an edge's confidence score to a color on a red -> orange "heat"
    scale for the PyVis graph: >=0.8 is the most saturated red (#C0392B,
    strongest claim), stepping down through #E74C3C and #E67E22 to the
    lightest orange #F39C12 for anything below 0.4 (the weakest claim that
    survived extract_causal_relations()'s own min_confidence=0.4 filter).
    Purely a display scale -- these thresholds are not statistically
    derived, just chosen to make higher-confidence edges visually "hotter."
    """
    if confidence >= 0.8:
        return '#C0392B'
    elif confidence >= 0.6:
...
def generate_causal_graph(causal_relations: list, output_path: str = "causal_graph.html") -> tuple:
    """
    Build a directed PyVis causal graph from causal relation dicts and
    render it to HTML.

    Node color encodes the node's role across all relations: purple if it
    appears as both a cause and an effect somewhere in the set (an
    intermediate variable), red if only ever a cause (a root cause), orange
    if only ever an effect (a terminal effect) -- see the node-role loop
    below. Node size scales with how central the variable is:
    20 + (times it appears as a cause * 5) + (times it appears as an
    effect * 3), capped at 50 -- cause-count is weighted higher since a
    variable driving many downstream effects is arguably more structurally
    important than one many things point to. Edge color comes from
    _confidence_to_color() above; edge width scales linearly with
    confidence (1 + confidence*3).

    Has a disk I/O side effect: net.save_graph(output_path) writes the
    HTML file, which is then immediately re-read back into a string
    (PyVis has no in-memory-only render path), so output_path must be a
    writable location.
    ...
    # Layout/style tuned empirically for readability with the typical
    # 10-40 node graphs this app produces, not PyVis defaults:
    # nodeDistance/springLength/springConstant control the force-directed
    # repulsion layout (larger nodeDistance/springLength spread nodes out
    # more, so labels don't overlap on denser graphs); scaleFactor sizes
    # the arrowhead; roundness bows edges slightly (curvedCW) so
    # bidirectional cause/effect pairs between the same two nodes don't
    # visually overlap into a single line.
    net.set_options("""{
        "physics": {
            "enabled": true,
            "repulsion": {
    ...
    # Counts how often each node appears as a cause vs. an effect across
    # every relation -- used below to decide each node's color (root
    # cause / intermediate / terminal effect) and its size.
    cause_count = {}
    effect_count = {}
    ...
    for node in all_nodes:
        ...
        # See generate_causal_graph()'s docstring above for the sizing rationale.
...
# Which of run_causal_discovery's methods actually consume tau_max / pc_alpha.
# Single source of truth for the frontend's conditional parameter widgets —
# must stay consistent with the `runners` dict's kwargs below.
TAU_MAX_METHODS = {'pcmci', 'pcmci_plus', 'tcdf', 'lpcmci'}
PC_ALPHA_METHODS = {'pc', 'pcmci', 'pcmci_plus', 'fci', 'cdnod', 'lpcmci'}
...
def run_causal_discovery(
    ...
    methods: any of 'pc', 'pcmci', 'pcmci_plus', 'tcdf', 'lingam', 'fci', 'cdnod', 'daggnn',
             'lpcmci', 'ges' -- see the Algorithm Glossary near the top of
             causal_discovery.py for what each one is, its category
             (constraint-based/functional/score-based/continuous-optimization/
             neural), and its source citation.
    ...
    Returns: {
        ...
        'full_graphs': {method_name: {'graph', 'val_matrix', 'var_names'}} —
                     only present for 'pcmci'/'pcmci_plus'/'lpcmci', the raw
                     Tigramite artifacts (every link type, not just resolved
                     edges) for plotting via
                     causal_discovery.plot_full_causal_graph().
    }
    """
    from causal_discovery import (
        run_pc, run_pcmci_full, run_pcmci_plus_full, run_tcdf, run_lingam, run_fci, run_cdnod,
        run_dag_gnn, run_lpcmci_full, run_ges, edges_to_kg_space
    )
    ...
    # 'pcmci'/'pcmci_plus'/'lpcmci' run through the "_full" variants below,
    # which compute exactly the same thing as before but also hand back the
    # raw Tigramite graph/val_matrix/var_names for the full-graph view — no
    # second run, just richer output from the one run already done.
    full_graph_methods = {'pcmci', 'pcmci_plus', 'lpcmci'}

    runners = {
        'pc': lambda: run_pc(dataset_df, mapped_columns, alpha=pc_alpha),
        'pcmci': lambda: run_pcmci_full(dataset_df, mapped_columns, tau_max=tau_max, alpha=pc_alpha),
        'pcmci_plus': lambda: run_pcmci_plus_full(dataset_df, mapped_columns, tau_max=tau_max, alpha=pc_alpha),
        ...
        # tau_max=5 on a 6-variable/1826-row dataset), so a user picking
        # tau_max=5 for PCMCI+/TCDF shouldn't also make LPCMCI take minutes
        # unasked.
        'lpcmci': lambda: run_lpcmci_full(dataset_df, mapped_columns, tau_max=min(tau_max, 2), alpha=pc_alpha),
    ...
    for method in methods:
        ...
        if method in full_graph_methods:
            edges = result['edges']
            full_graphs[method] = {
                'graph': result['graph'],
