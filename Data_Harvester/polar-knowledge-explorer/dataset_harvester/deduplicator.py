...
# A "name" that is really a URL / DOI / bare accession. These are unique
# identifiers, not human-readable names — they must only ever match by the
# exact DOI/URL/accession indexes, never by fuzzy string similarity (otherwise
# distinct records like 10.5884/12518 and 10.5884/12519 collapse into one).
_IDENTIFIER_LIKE = re.compile(
    r'(?:://|^https?:|^www\.|^doi:|^10\.\d{4,}/|pangaea\.\d|zenodo\.\d|nsidc-\d)',
    re.IGNORECASE,
)


def _looks_like_identifier(name: str) -> bool:
    return bool(_IDENTIFIER_LIKE.search(name or ""))


def _same_dataset(name_a: str, name_b: str) -> bool:
    # If either "name" is actually an identifier string, only an exact match on
    # the identifier indexes (steps 1-3 upstream) may merge them. By the time
    # we reach fuzzy name matching, distinct identifiers must stay separate.
    if _looks_like_identifier(name_a) or _looks_like_identifier(name_b):
        return False
...
class DeduplicatedDataset:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "used_in_study": self.used_in_study,
        }
...
def _merge_into(existing: DeduplicatedDataset, ref: DatasetRef, source: str):
    ...
    # Any mention marked primary or used_in_study promotes the whole group
    if ref.is_primary:
        existing.is_primary = True
    if ref.used_in_study:
        existing.used_in_study = True
        ...
        for ref in refs:
            ...
            if idx is not None:
                ...
                d = DeduplicatedDataset(
                    ...
                    used_in_study=ref.used_in_study,
                )
                idx = len(deduped)
                deduped.append(d)
                if d.doi:
