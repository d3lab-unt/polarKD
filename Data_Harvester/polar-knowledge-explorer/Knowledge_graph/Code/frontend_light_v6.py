...
try:
    ...
    from keywords_extraction import process as process_enhanced
...
from causal_graph import extract_causal_relations, generate_causal_graph, run_causal_discovery, is_valid_node
from causal_discovery import (
    load_dataset, detect_time_column, numeric_columns, suggest_variable_mapping, clean_node_name,
    prettify_node_name, prettify_column_name,
)
import os
import sys
import re
    ...
    if repo == "pangaea" or "pangaea" in (r.resolved_url or "").lower():
        ...
        if pid:
            ...
            for attempt in range(3):
                try:
                    resp = requests.head(
                        ...
                        headers={"User-Agent": "arctic-dataset-harvester/1.0"},
                    )
                    last_status = resp.status_code
                    if resp.status_code == 200:
                        return "yes", "Downloadable", "PANGAEA file"
                    if resp.status_code in (429, 500, 502, 503, 504):
                        # Transient server-side issue — not proof it's a
                        # collection, just busy/rate-limited. Retry.
            ...
            if last_status in (400, 404):
                # Only a durable "not a single downloadable file" response
                # implies a parent/collection record — anything else (429,
                # 5xx, timeouts) is a transient hiccup, not evidence of
                # having child datasets.
                return "collection", "Collection", "Parent record — contains child datasets"
            return "unknown", "Unknown", "Could not reach PANGAEA (may be temporarily busy)"
...
def _fetch_collection_children(pid: str) -> list[dict]:
    ...
    try:
        resp = requests.get(
            ...
            timeout=20, headers={"Accept": "text/html,*/*"}, allow_redirects=True,
        )
        if resp.status_code == 200:
            child_ids = sorted(set(re.findall(r'PANGAEA\.(\d+)', resp.text)) - {pid})
            if child_ids:
                children = [
                    {"name": f"PANGAEA.{cid}", "id": cid,
                     "url": f"https://doi.pangaea.de/10.1594/PANGAEA.{cid}", "downloadable": False}
...
def _convert_to_csv(fpath: str) -> str:
    """Convert a .tab/.txt/.tsv file to .csv saved alongside the original.
    Returns the .csv path on success, or the original path if already CSV / conversion fails."""
    name_lower = fpath.lower()
    # Strip .gz wrapper for extension check
    base = name_lower[:-3] if name_lower.endswith(".gz") else name_lower
    ext = os.path.splitext(base)[1]
    if ext == ".csv":
        return fpath  # already CSV
    if ext not in (".txt", ".tsv", ".tab"):
        return fpath  # unknown format — leave as-is
    csv_path = os.path.splitext(fpath)[0] + ".csv"
    if os.path.exists(csv_path):
        return csv_path  # already converted in a prior run
    try:
        with _open_possibly_gz(fpath) as _f:
            lines = _f.readlines()
        # Skip PANGAEA /* ... */ header block
        start = 0
        in_header = False
        ...
        # Reject if columns look garbled (binary file read as text)
        if df.empty or df.shape[1] < 2 or any(
            any(c not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 _-./()[]°%‰±<>~" for c in str(col))
        ...
        return fpath  # conversion failed — return original
        ...
        if ext in (".txt", ".tsv", ".tab"):
            ...
            for idx, line in enumerate(lines):
                ...
                if in_header and s.endswith("*/"):
                    ...
                    break
            data_text = "".join(lines[start:])
            return pd.read_csv(io.StringIO(data_text), sep="\t", nrows=n, on_bad_lines="skip")
        if ext == ".csv":
            with _open_possibly_gz(fpath) as _f:
                content = _f.read()
            return pd.read_csv(io.StringIO(content), nrows=n, on_bad_lines="skip")
    except Exception:
        return None
    return None


def _load_full_dataset_from_disk(fpath: str) -> "pd.DataFrame | None":
    """Load a downloaded dataset file as a full DataFrame for causal analysis."""
    name = fpath.lower()
    if name.endswith(".gz"):
        name = name[:-3]
    ext = os.path.splitext(name)[1]
        ...
        if ext in (".txt", ".tsv", ".tab"):
            ...
            for idx, line in enumerate(lines):
                ...
                if in_header and s.endswith("*/"):
                    ...
                    break
            data_text = "".join(lines[start:])
            return pd.read_csv(io.StringIO(data_text), sep="\t", on_bad_lines="skip")
        if ext == ".csv":
            with _open_possibly_gz(fpath) as _f:
                content = _f.read()
...
# Enhanced Structural Causal Graph's (Section 4) method multiselect and
...
# rendering and fuzzy variable-name matching (Section 4) would rebuild on
...
for key, default in [
    ...
    # Enhanced Structural Causal Graph pipeline state — entirely separate
    # from the keys above by design, so this pipeline never reads or writes
    # Section 1/3's state and vice versa.
    ...
    ('scg_results', None),
    # Dataset harvester state
    ('harvester_cache_key', None),
    ('harvester_results', None),
    ('harvester_elapsed', None),
    # Variable tabulation state
    ('tabulator_cache_key', None),
    ('tabulator_results', None),
    ('dl_results', {}),
    ('extraction_started', False),
    ('extraction_cache_key', None),
]:
...
with col1:
    ...
    # Re-uploading a different set of files requires a fresh "Dataset Extraction" click
    _extraction_cache_key = (
        "_".join(f"{f.name}:{f.size}" for f in uploaded_files) if uploaded_files else None
    )
    if st.session_state.get("extraction_cache_key") != _extraction_cache_key:
        st.session_state.extraction_cache_key = _extraction_cache_key
...
#                 st.session_state.show_qa_dialog = False
    ...
    if st.button("🧬 Generate Structural Causal Graph", use_container_width=True, key="gen_scg"):
        if not (uploaded_files and len(uploaded_files) > 0):
            ...
            # scratch on every click (same as Section 1's KG button) — a repeat
    ...
    if st.session_state.show_scg_kg_dialog:
        ...
        ">
            ...
            <div style="font-size:0.76rem;color:#5F7A9D;letter-spacing:0.04em;">
                Independent KG extraction for Section 6 — choose model and graph type
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.session_state.scg_cg_relations:
        ...
        # ── Downloaded datasets picker ────────────────────────────────────────
        _dl_csv_files = [
            fp
            for _v in st.session_state.dl_results.values()
            if _v[0] == "ok"
            for fp in _v[1]
            if fp.lower().endswith(".csv")
        ]
        if _dl_csv_files:
            st.markdown("**Or select a downloaded dataset:**")
            for _dlf in _dl_csv_files:
                _dlf_id = f"dl_{hash(_dlf)}"
                _is_active = st.session_state.scg_validation_dataset_file_id == _dlf_id
                _label = f"{'✓ ' if _is_active else ''}{os.path.basename(_dlf)}"
                if st.button(_label, key=f"scg_pick_dl_{hash(_dlf)}", use_container_width=True):
                    _df_picked = _load_full_dataset_from_disk(_dlf)
                    if _df_picked is not None and not _df_picked.empty:
                        st.session_state.scg_validation_dataset = _df_picked
                        st.session_state.scg_validation_dataset_name = os.path.basename(_dlf)
                        st.session_state.scg_validation_dataset_file_id = _dlf_id
                        st.session_state.scg_discovery_dialog_handled_for = None
                        st.session_state.scg_results = None
                        st.rerun()
                    else:
                        st.error(f"Could not read {os.path.basename(_dlf)} as a tabular dataset.")
        ...
        if st.session_state.show_scg_discovery_dialog:
            ...
            with col_confirm:
                if st.button("✓ Confirm", use_container_width=True, key="scg_discovery_confirm"):
                    st.session_state.show_scg_discovery_dialog = False
                    st.session_state.scg_run_request = {'methods': st.session_state.scg_methods_selected, 'mapping': scg_mapping}
                    st.rerun()
            with col_cancel:
    ...
    # ── Dataset Extraction Button (gates the harvester + tabulator sections below)
    if st.button(
        "Dataset Extraction",
        use_container_width=True,
        key="start_extraction",
        disabled=not (uploaded_files and len(uploaded_files) > 0),
        help="Runs the dataset harvester and variable tabulator on the uploaded PDF(s).",
    ):
        st.session_state.extraction_started = True


# ══════════════════════════════════════════════════════════════════════════
#  SECTION 1b — DATASET HARVESTER (runs after "Dataset Extraction" is clicked)
# ══════════════════════════════════════════════════════════════════════════
st.markdown("---")
...
if not _HARVESTER_AVAILABLE:
...
elif uploaded_files and len(uploaded_files) > 0 and st.session_state.get("extraction_started"):
  ...
  {"<a href='" + p['URL'] + "' target='_blank' style='color:#2E5FA0;font-size:0.85rem'>" + p['URL'][:90] + ("…" if len(p['URL']) > 90 else "") + "</a>" if p['URL'] else "<em style='color:#888;font-size:0.85rem'>URL not resolved</em>"}
                ...
                if can_dl:
                    if st.button("Download to disk", key=f"dl_btn_{i}"):
                        with st.status("Downloading…", expanded=True) as _dl_status:
                            try:
                                ...
                                # Auto-convert .tab/.txt/.tsv → .csv for uniform handling
                                files = [_convert_to_csv(f) for f in files]
                                st.session_state.dl_results[result_key] = ("ok", files)
                                _dl_status.update(label=f"Downloaded {len(files)} file(s)", state="complete", expanded=False)
                            except Exception as _dl_err:
                                st.session_state.dl_results[result_key] = ("err", str(_dl_err))
                                _dl_status.update(label=f"Download failed: {_dl_err}", state="error", expanded=False)
                    ...
                    url = p.get("URL", "")
                    if url:
                        st.info("**Open access** — no in-app download, but the data is publicly available. Click the link above to go to the data portal and download manually.")
                    else:
                    ...
                    if children:
                        ...
                        for j, c in enumerate(children):
                                ...
                                if c.get("downloadable") and st.button("⬇ Download", key=_c_dl_key):
                                    ...
                                    with st.spinner(f"Downloading {c['name']}…"):
                                        try:
                                            ...
                                            # Auto-convert .tab/.txt/.tsv → .csv
                                            _child_files = [_convert_to_csv(f) for f in _child_files]
                            ...
                            if _c_res_key in st.session_state:
                                ...
                                if _c_ok == "ok":
                                    st.success(f"{len(_c_data)} file(s) saved (auto-converted to CSV where applicable):")
                                    for _cfpath in _c_data:
                                        st.code(_cfpath)
                                        _c_ext_check = os.path.splitext(_cfpath.lower())[1]
                                        if _c_ext_check not in _CSV_EXTS:
                                            st.info(f"Binary/unsupported format ({_c_ext_check or 'unknown'}) — saved to disk but not previewable as a table.")
                                            continue
                                        ...
                                        if _cdf is not None and not _cdf.empty:
                                            ...
                                            try:
                                                _cdf_full = _load_full_dataset_from_disk(_cfpath)
                                                _cbytes = (_cdf_full if _cdf_full is not None else _cdf).to_csv(index=False).encode()
                                                st.download_button(
                                                    "Download CSV",
                                                    data=_cbytes,
                                                    file_name=os.path.splitext(os.path.basename(_cfpath))[0] + ".csv",
                                                    mime="text/csv",
                                                    key=f"csv_dl_child_{hash(_cfpath)}",
                                                )
                                            except Exception:
                                                pass

                                        # ── Send to Causal Graph bridge ──────
                                        _c_fname_lower = _cfpath.lower()
                                        _c_ext_raw = os.path.splitext(_c_fname_lower)[1]
                                        if _c_ext_raw in (".csv", ".txt", ".tsv", ".tab"):
                                            _c_cg_btn_key = f"cg_bridge_child_{hash(_cfpath)}"
                                            if st.button("⟶ Send to Causal Graph Analysis", key=_c_cg_btn_key,
                                                         help="Load this dataset into the Structural Causal Graph pipeline below"):
                                                _c_df_full = _load_full_dataset_from_disk(_cfpath)
                                                if _c_df_full is not None and not _c_df_full.empty:
                                                    st.session_state.scg_validation_dataset = _c_df_full
                                                    st.session_state.scg_validation_dataset_name = os.path.basename(_cfpath)
                                                    st.session_state.scg_validation_dataset_file_id = f"dl_child_{hash(_cfpath)}"
                                                    st.session_state.scg_discovery_dialog_handled_for = None
                                                    st.session_state.scg_results = None
                                                    st.rerun()
                                                else:
                                                    st.error("Could not parse this file as a tabular dataset.")
                                else:
                ...
                # Download result + preview + Send to Causal Graph bridge
                if result_key in st.session_state.dl_results:
                    ok, data = st.session_state.dl_results[result_key]
                    if ok == "ok":
                        st.success(f"{len(data)} file(s) saved to disk (auto-converted to CSV where applicable):")
                        for fpath in data:
                            st.code(fpath)
                            _ext_check = os.path.splitext(fpath.lower())[1]
                            if _ext_check not in _CSV_EXTS:
                                st.info(f"Binary/unsupported format ({_ext_check or 'unknown'}) — saved to disk but not previewable as a table.")
                                continue
                            df_preview = _preview_file(fpath)
                            if df_preview is not None and not df_preview.empty:
                                st.caption(f"First {len(df_preview)} row(s) — {os.path.basename(fpath)}")
                                st.dataframe(df_preview, use_container_width=True, hide_index=True)
                                # ── In-browser CSV download button (full dataset, not just preview) ──
                                try:
                                    _df_full_dl = _load_full_dataset_from_disk(fpath)
                                    _csv_bytes = (_df_full_dl if _df_full_dl is not None else df_preview).to_csv(index=False).encode()
                                    st.download_button(
                                        "Download CSV",
                                        data=_csv_bytes,
                                        file_name=os.path.splitext(os.path.basename(fpath))[0] + ".csv",
                                        mime="text/csv",
                                        key=f"csv_dl_{hash(fpath)}",
                                    )
                                except Exception:
                                    pass
                            ...
                            _ext_raw = os.path.splitext(_fname_lower)[1]
                            if _ext_raw in (".csv", ".txt", ".tsv", ".tab"):
                                _cg_btn_key = f"cg_bridge_{hash(fpath)}"
                                if st.button("⟶ Send to Causal Graph Analysis", key=_cg_btn_key,
                                             help="Load this dataset into the Structural Causal Graph pipeline below"):
                                    _df_full = _load_full_dataset_from_disk(fpath)
                                    if _df_full is not None and not _df_full.empty:
                                        st.session_state.scg_validation_dataset = _df_full
                                        st.session_state.scg_validation_dataset_name = os.path.basename(fpath)
                                        st.session_state.scg_validation_dataset_file_id = f"dl_{hash(fpath)}"
                                        st.session_state.scg_discovery_dialog_handled_for = None
                                        st.session_state.scg_results = None
                                        st.rerun()
                                    else:
    ...
    st.markdown(
        ...
        'Click <strong>Dataset Extraction</strong> above to find dataset references in the uploaded PDF(s).</div>',
        unsafe_allow_html=True
    )
else:
    st.markdown(
        '<div style="padding:1rem;background:#F0F4FA;border-radius:6px;color:#5F7A9D;font-size:0.85rem;">'
        'Upload PDFs above, then click <strong>Dataset Extraction</strong> to see dataset references here.</div>',
        unsafe_allow_html=True
    )
...
if not _TABULATOR_AVAILABLE:
    ...
    if _tab_results:
        for _tr in _tab_results:
            ...
            with st.expander(f"{_icon} {_paper_label}", expanded=False):
                ...
                if _has_vars:
                    _rows = [
                        {"Variable": v["name"], "Unit": v["unit"], "Source": v.get("source", "—")}
    ...
    st.markdown(
        '<div style="padding:1rem;background:#F0F4FA;border-radius:6px;color:#5F7A9D;font-size:0.85rem;">'
        'Click <strong>Dataset Extraction</strong> above to extract variables and coverage from the uploaded PDF(s).</div>',
        unsafe_allow_html=True,
    )
else:
    st.markdown(
        '<div style="padding:1rem;background:#F0F4FA;border-radius:6px;color:#5F7A9D;font-size:0.85rem;">'
        'Upload PDFs above, then click <strong>Dataset Extraction</strong> to see variable coverage here.</div>',
        unsafe_allow_html=True,
    )
...
# trigger/result split Section 3 already uses. This block only reads scg_*
